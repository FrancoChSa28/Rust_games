...
impl Game {
    ...
    pub fn run(&mut self) {
        ...
        while !done {
            ...
            while self.is_paused {
                if let Some(command) = self.get_command(Duration::from_secs(1)) {
                    match command {
                        Command::Pause => {
        ...
        println!("Game Over! Your best score was {}", self.best_score);
    }
    ...
    fn get_command(&self, wait_for: Duration) -> Option<Command> {
        ...
        match key_event.code {
            ...
            KeyCode::Enter => Some(Command::Pause),
            _ => None
        }
    }
    ...
    fn draw_life(&mut self){
        self.stdout
            .execute(MoveTo(1, self.height + 3)).unwrap()
            .execute(Print(format!("Lives: {}", self.lives))).unwrap();
    }

    fn drwa_best_score(&mut self) {
        self.stdout
            .execute(MoveTo(1, self.height + 4)).unwrap()
            .execute(Print(format!("Best Score: {}", self.best_score))).unwrap();
    }
}